...
HabitatLayer = rast(paste0("inputs/habitat_", Landscape,".tif"))
plot(HabitatLayer)
unique(values(HabitatLayer))

# ### If necessary add fragmenting elements on top, e.g. roads and rivers, as shapefile
# # Has to be shapefile, croped at the study area
# # If you don't have / need fragmenting elements, please do not set up this parameter
# Road = vect(paste0("inputs/roads_",Landscape,".shp"))
# River = vect(paste0("inputs/rivers_", Landscape, ".shp"))
...
TooSmallPatches = rast(paste0("patches_too_small_",Landscape,".tif"))
# correct-size patches are saved at (stored with a patch number):  
CorrectPatches = rast(paste0("patches_correct_size_",Landscape,".tif"))
# too-large patches are saved at (0,1 layer):  
TooLargePatches = rast(paste0("patches_too_large_", Landscape, ".tif"))
...
FinalPatches = rast(paste0("final_patches_", Landscape,".tif"))
